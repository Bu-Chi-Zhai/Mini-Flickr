...
app.controller("PhotoCtrl", ["$http", "$scope", "$filter", function ($http, $scope, $filter) {
    $http.get("/api/v1/photos")
        .then(function (response) {
            ...
            $scope.searchTagsByBtn = function(tagsFilter){
                if ($scope.pinFilter === 'all') {
                    $scope.filteredPhotos = $filter('filter')($scope.photos, {tags: tagsFilter});
                } else if ($scope.pinFilter === 'pinned') {
                    $scope.filteredPhotos = $filter('filter')($scope.photos, {pinned: true, tags: tagsFilter});
                }
            }
